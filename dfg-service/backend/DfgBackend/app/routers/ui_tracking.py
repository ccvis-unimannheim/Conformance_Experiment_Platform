...
async def post_ui_log_data(ui_log_data: ds.UILogDataFrontend, dfg_user_id: Annotated[str | None, Cookie()] = None):
    if dfg_user_id is None:
        return JSONResponse(content={"message": "No cookie detected! Please call GET /auth to receive a cookie"})
    ui_log_data_db = ds.UILogDataDatabase(user_id=dfg_user_id, ui_log_data=ui_log_data)

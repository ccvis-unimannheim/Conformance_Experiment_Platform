  ...
  async function publishExperiment() {
    try {
      const res = await fetch(
        `${BASE_URL}/admin/experiments/${experimentId}/status?status=active`,
        { method: "PATCH" }
      );
      if (!res.ok) throw new Error(await res.text());
      showToast("Experiment published! Status set to active.");
      setSuccessDetail((prev) => prev.replace("saved", "published (active)"));
    } catch (e) {

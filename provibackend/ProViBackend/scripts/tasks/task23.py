...
from shared import (
    ...
    BLUE, ORANGE, RED, FONT_TITLE, FONT_LABEL, FONT_ANNOT,
)
...
def _task23_build_pattern_df(alignments) -> pd.DataFrame:
    """Build per-pattern DataFrame from alignment results.

    Columns: pattern, activity, move_type, count, n_traces, pct
    Sorted by count descending.
    Pattern = "activity (move_type)", consistent with task05.
    """
    rows = []
    for trace_idx, result in enumerate(alignments):
        for step in alignment_pairs_to_rows(result.get("alignment", [])):
            ...
            if not activity or activity in {"-", "None"}:
                continue
            rows.append({
...
def task23_bar_chart(pat_df: pd.DataFrame, output_dir: str):
    ...
    colors = [_MOVE_COLORS.get(mt, BLUE) for mt in top["move_type"]]
...
def task23_table_and_bar_chart(pat_df: pd.DataFrame, output_dir: str):
    ...
    ax_bar.legend(
        ...
        frameon=False, fontsize=FONT_ANNOT,
        loc="lower right", bbox_to_anchor=(1.0, -0.18), ncol=1,
    )

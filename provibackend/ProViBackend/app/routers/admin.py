...
try:
    from ProViBackend.scripts import create_all_visualizations
except ImportError:
...
def _run_pair_pipeline(pair_dir: pl.Path, pair_id: str):
    """Background task: generate all SVGs for an uploaded dataset pair."""
    if create_all_visualizations is None:
        _pipeline_logger.error("create_all_visualizations module not available; skipping pipeline.")
        return
    try:
        dbc.connect_to_database()["DatasetPair"].update_one(
            {"dataset_id": pair_id}, {"$set": {"pipeline_status": "running"}}
        )
        create_all_visualizations.run_pipeline(
            dataset_dir=str(pair_dir),
            output_dir=str(pair_dir),
        )
        dbc.connect_to_database()["DatasetPair"].update_one(
            {"dataset_id": pair_id}, {"$set": {"pipeline_status": "done"}}
        )
    except Exception as exc:
        _pipeline_logger.error(f"Pipeline failed for {pair_dir}: {exc}", exc_info=True)
        dbc.connect_to_database()["DatasetPair"].update_one(
            {"dataset_id": pair_id}, {"$set": {"pipeline_status": "failed", "pipeline_error": str(exc)}}
        )
...
async def upload_dataset_pair(log: UploadFile, guideline: UploadFile,
    ...
    # Files are saved under input/ with canonical names expected by run_pipeline
    input_dir = pair_dir / "input"
    try:
        input_dir.mkdir(parents=True, exist_ok=True)

        # Detect extension and use canonical filenames
        log_ext  = pl.Path(log.filename or "event.xes").suffix.lower() or ".xes"
        log_canonical  = f"EventLog{log_ext}"
        guid_canonical = "Guideline.bpmn"

        log_contents = await log.read()
        log_path = input_dir / log_canonical
        log_path.write_bytes(log_contents)

        guideline_contents = await guideline.read()
        guideline_path = input_dir / guid_canonical
        ...
        background_tasks.add_task(_run_pair_pipeline, pair_dir, pair_id)
